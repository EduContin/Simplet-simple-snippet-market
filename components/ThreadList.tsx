...
interface ThreadListProps {
  threads: Thread[];
  view?: "table" | "cards";
  previews?: Record<number, { contentSnippet: string; title?: string; language?: string; metaTags?: string[]; metaLicense?: string | null; metaPriceLabel?: string } | undefined>;
...
const limitTitle = (title: string, maxLength: number = 70): string => {
  if (title.length > maxLength) return title.slice(0, maxLength - 3) + "...";
  return title;
};
...
const ThreadList: React.FC<ThreadListProps> = ({ threads, view = "table", previews, linkResolver, onDismiss }) => {
  ...
  const visibleThreads = useMemo(() => sortedThreads.filter((t) => !dismissed[t.id]), [sortedThreads, dismissed]);
  ...
  const toggleLike = async (thread: Thread) => {
    try {
      ...
      const lr = await fetch('/api/v1/likes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postId, userId }) });
      if (!lr.ok) return;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visibleThreads.map((thread) => {
            ...
            const derivedFromCode = (() => {
              ...
              if (/^\s*(\/\/|#|--|\/\*|\*)/.test(firstLine)) return firstLine.replace(/^\s*(\/\/|#|--|\/\*|\*)\s*/, "").slice(0, 60);
              if (/function\s+([a-zA-Z0-9_]+)/.test(firstLine)) { const m = firstLine.match(/function\s+([a-zA-Z0-9_]+)/); return m ? `${m[1]}()` : undefined; }
              if (/class\s+([a-zA-Z0-9_]+)/.test(firstLine)) { const m = firstLine.match(/class\s+([a-zA-Z0-9_]+)/); return m ? `${m[1]} class` : undefined; }
              if (/def\s+([a-zA-Z0-9_]+)/.test(firstLine)) { const m = firstLine.match(/def\s+([a-zA-Z0-9_]+)/); return m ? `${m[1]}()` : undefined; }
              return undefined;
            })();
            const titleToShow = limitTitle(titleOverride || derivedFromCode || thread.title, 60);
            const isExpanded = !!expanded[thread.id];
            return (
              <Link key={thread.id} href={resolvedHref} className="group block rounded-lg border border-gray-600/40 bg-gray-700/40 hover:bg-gray-700/60 transition-colors duration-200 overflow-hidden snippet-ide-shadow">
                <div className="px-4 pt-4 pb-2">
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    <button type="button" aria-label="Dismiss snippet" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setDismissed((d) => ({ ...d, [thread.id]: true })); onDismiss?.(thread.id); }} className="h-3 w-3 rounded-full bg-red-500 hover:opacity-80" />
                    <button type="button" aria-label="Expand snippet" aria-expanded={!!expanded[thread.id]} onClick={(e) => { e.preventDefault(); e.stopPropagation(); setExpanded((ex) => ({ ...ex, [thread.id]: !ex[thread.id] })); }} className="h-3 w-3 rounded-full bg-green-500 hover:opacity-80" />
                    ...
                    <h3 className="text-sm font-semibold text-blue-300 group-hover:text-blue-200 truncate flex items-center gap-2" title={titleToShow}>
                      {titleToShow}
                      {(thread as any).is_verified && (<span className="inline-flex items-center text-xs text-green-400 border border-green-500/50 rounded px-1 py-0.5">Verified</span>)}
                      {((thread as any).file_count ?? 0) > 1 && (<span className="inline-flex items-center text-[10px] text-gray-300 border border-gray-500/40 rounded px-1 py-0.5">{(thread as any).file_count} files</span>)}
                    </h3>
                  </div>
                </div>
                {(Array.isArray(thread.meta_tags) && thread.meta_tags.length > 0) || thread.meta_license || typeof thread.meta_price_label === 'string' || (previews?.[thread.id]?.metaTags?.length || 0) > 0 || previews?.[thread.id]?.metaLicense || typeof previews?.[thread.id]?.metaPriceLabel === 'string' ? (
                  <div className="px-4 pb-2 -mt-1 flex flex-wrap items-center gap-1">
                    {((Array.isArray(thread.meta_tags) && thread.meta_tags.length > 0) ? thread.meta_tags : previews?.[thread.id]?.metaTags || []) && (
                      <div className="flex items-center gap-1">
                        {(Array.isArray(thread.meta_tags) && thread.meta_tags.length > 0 ? thread.meta_tags.slice(0,4) : (previews?.[thread.id]?.metaTags || []).slice(0,4)).map((t: string) => (<span key={t} className="text-[10px] border border-gray-600/60 rounded px-1 py-0.5 text-gray-300">{t}</span>))}
                      </div>
                    )}
                    {(thread.meta_license || previews?.[thread.id]?.metaLicense) && (<span className="text-[11px] text-amber-300 border border-amber-500/40 rounded px-1 py-0.5">{thread.meta_license || previews?.[thread.id]?.metaLicense}</span>)}
                    {typeof (thread.meta_price_label ?? previews?.[thread.id]?.metaPriceLabel) === 'string' && (<span className="text-[11px] text-emerald-300 border border-emerald-500/40 rounded px-1 py-0.5">{thread.meta_price_label ?? previews?.[thread.id]?.metaPriceLabel}</span>)}
                  </div>
                ) : null}
                <div className="px-4">
                      ...
                      <div className={`p-3 overflow-hidden ${isExpanded ? 'max-h-80' : 'max-h-40'} transition-[max-height] duration-500 ease-in-out`}>
                        {preview ? (
                          <pre className={`font-mono text-[12px] leading-5 text-gray-200 whitespace-pre-wrap break-words ${isExpanded ? 'max-h-72' : 'max-h-32'} overflow-hidden transition-[max-height] duration-500 ease-in-out`}>
                            <code className={`language-${previewLang || mapCategoryToLanguage(thread.category_name)}`}>{preview}</code>
                          </pre>
                        ) : (
                ...
                <div className="px-4 pb-4 mt-3 flex items-center justify-between text-gray-300 text-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <button type="button" aria-label="Like snippet" onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleLike(thread); }} className={`flex items-center gap-1 ${likedMap[thread.id] ? 'text-red-400' : 'text-gray-300 hover:text-red-300'}`}>
          ...
          <Link href="/snippet/new" className="group flex items-center justify-center rounded-lg border border-dashed border-gray-600/60 bg-gray-700/30 hover:bg-gray-700/50 transition-colors duration-200 min-h-[260px] snippet-ide-shadow" aria-label="Add new snippet">
        ...
        <table className="w-full">
          ...
          <tbody>
            {sortedThreads.map((thread) => (
              <tr key={thread.id} className="hover:bg-gray-700/50 transition-colors duration-200">
                <td className="py-3 px-4 border-b border-gray-600/50">
                  <Link href={`/thread/${slugify(thread.title)}-${thread.id}`} className="text-blue-400 hover:text-blue-300 flex items-center">
                    <FaComments className="mr-2 flex-shrink-0" />
                    <span className="truncate" title={thread.title}>{limitTitle(thread.title)}</span>
                  </Link>
                </td>
                ...
                <td className="py-3 px-4 border-b border-gray-600/50 text-gray-300">{thread.post_count - 1}</td>
                <td className="py-3 px-4 border-b border-gray-600/50 text-gray-300">
                  <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleLike(thread); }} className={`flex items-center ${likedMap[thread.id] ? 'text-red-400' : 'text-gray-300 hover:text-red-300'}`} aria-label="Like snippet">
